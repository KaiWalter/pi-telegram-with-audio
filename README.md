# pi-telegram-with-audio (fork)

> ⚠️ **Fork notice:** this repository is a maintained fork of the original
> [`badlogic/pi-telegram`](https://github.com/badlogic/pi-telegram).
> Keep `origin` on this fork and `upstream` on the original so upstream pulls
> can be re-adapted cleanly.

## Additional capabilities in this fork

- Folded local audio I/O extension (`folded/audio-io`) for voice transcription flow.
- Folded new-session bridge (`folded/new-session`) for `/new` Telegram interception.
- Folded image I/O helper surface (`folded/image-io`) used by inbound image handlers.
- Folded reboot bridge (`folded/reboot-bridge`) for immediate reboot-intent handling (lane-configurable).
- Sandbox-first adaptation model with local provenance via `SNAPSHOT.md`.
